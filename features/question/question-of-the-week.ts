/**
 * One question this week — deterministic week -> question mapping.
 *
 * Mirrors `packages/shared/src/question.ts` (the API uses the same bank and
 * the same math, so both partners — and stub mode — always land on the same
 * question for the same ISO week). Strictly optional ritual: no
 * notifications, no badges, no pressure — the mapping simply exists.
 */

/**
 * The seeded question bank. Handcrafted, warm, concrete. The mapping cycles
 * through these forever — they are written to bear gentle repetition.
 */
...
/**
 * Deterministic week -> question mapping, identical for both partners: an
 * ever-increasing week ordinal (mod the question count). Consecutive weeks
 * always get consecutive questions, except across a 53-week ISO year
