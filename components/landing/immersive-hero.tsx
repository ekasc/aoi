...
export function ImmersiveHero({ cta }: ImmersiveHeroProps) {
  ...
  const warmGlow = useThemeColor(
    { light: 'rgba(232, 138, 112, 0.22)', dark: 'rgba(232, 138, 112, 0.18)' },
    'partnerAccent'
  );
  ...
  const gloss = useThemeColor(
    { light: 'rgba(255, 255, 255, 0.22)', dark: 'rgba(255, 255, 255, 0.1)' },
    'surface'
  );
  ...
  }, [ambientDriftSoft, ambientDriftSun, ambientDriftX, ambientDriftY, reduceMotion]);
  ...
  return (
    <View style={styles.heroWrap}>
      ...
      >
        <GlassSurface effect='regular' style={[styles.heroCard, { borderColor: border }]}>
          <View pointerEvents='none' style={[styles.glossOverlay, { backgroundColor: gloss }]} />
          <ThemedText type='meta' selectable={false} style={{ color: muted }}>
            Private for two
          </ThemedText>
          <ThemedText
            type='display'
            selectable={false}
            style={[styles.display, isCompact ? styles.displayCompact : undefined]}
          >
            Aoi
          </ThemedText>
          <ThemedText
            type='title'
            selectable={false}
            style={[styles.tagline, { color: text }, isCompact ? styles.taglineCompact : undefined]}
          >
            The space between the tide and the shore
          </ThemedText>
          <View style={[styles.divider, { backgroundColor: border }]} />
          <ThemedText type='body' selectable={false} style={{ color: muted }}>
