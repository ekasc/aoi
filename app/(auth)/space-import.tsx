...
export default function SpaceImportScreen() {
  ...
  const handleSave = useCallback(async () => {
    ...
    if (preparedInputs.length === 0) {
      setError('Add at least one milestone or skip for now.');
      return;
    }
      ...
      >
        ...
        >
          ...
          {rows.map((row, index) => {
            ...
            const typeChipColor = row.type === 'goal' ? accent : border;
            return (
                    ...
                    return (
                      <Pressable
                        ...
                        style={[
                          styles.typeChip,
                          {
                            borderColor: isSelected ? typeChipColor : border,
        ...
        <View style={[footerStyle, { borderColor: border, backgroundColor: background }]}>
          <Button
            disabled={isSaving}
            label={isSaving ? 'Importing…' : 'Continue'}
            onPress={handleSave}
          />
