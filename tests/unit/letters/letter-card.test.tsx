...
vi.mock('react-native', () => {
  ...
  const View = ({ children, style, ...rest }: Record<string, unknown>) =>
    React.createElement('div', { style: flatten(style), ...rest }, children as never);
...
vi.mock('@/components/ui/surface', () => ({
  Surface: ({ children, style }: { children?: unknown; style?: unknown }) => {
    const React = require('react');
    const flat = Array.isArray(style)
      ? Object.assign({}, ...style.filter(Boolean))
      : style;
    return React.createElement('div', { style: flat, 'data-testid': 'surface' }, children);
  },
}));
...
describe('LetterCard', () => {
  ...
  describe('opened letters', () => {
    ...
    it('finally renders the words in the card', () => {
      render(
        <LetterCard
          letter={makeLetter({
            isOpened: true,
    ...
    it('is not pressable once opened', () => {
      const onPress = vi.fn();
      const { container } = render(
        <LetterCard
          letter={makeLetter({
      ...
      // Opened letters render a plain surface, not a pressable card.
      fireEvent.click(container.firstChild as Element);
